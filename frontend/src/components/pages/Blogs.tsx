    ...
    return <div>
        {
            blogs.blogs.map((blog : BlogType) => {
                return <div key={key++} className='mb-[10px]' onClick={() => {
                    localStorage.setItem('postId', blog.id.toString());
                    navigate('/read');
                }}>
                    <BlogCard avatar={blog.author.name[0]} author={blog.author.name} date={blog.createdAt} title={blog.title} content={htmlToString(blog.content)}/>
                </div>
            })
        }
    </div>
...
function BlogCard({avatar, author, date, title, content}: BlogCardType){
